...
时间，以及收藏数（按源 + 按文章），全部来自已有归档表，无需新埋点。
...
def summarize(session: Session, *, top_n: int = 12) -> Dict[str, Any]:
    ...
    totals_articles = 0
    for source_id, content_type, count, last_fetched in article_rows:
        if not source_id:
            continue
