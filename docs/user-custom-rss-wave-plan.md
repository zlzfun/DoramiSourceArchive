...
> 2026-09-04 现势修订：`user_rss_` 表示“用户添加、目录不收录”，不再等同于
> “内网私有资产”。普通公开 RSS 在内网采集并调用 MaaS 分析，可产生只供审核的
> 全局 Candidate；含 token/signature 等凭证的 RSS 默认禁止 MaaS 与公开分享。
> 自定源正文不经 Archive Sync 上传外网 Dorami。本文其余“全部私有/全部禁用分析”
> 表述是 v3.40 的历史设计背景，以本修订和现行代码为准。
