...
- **DB 分流**(目标上下文的迁移计划):目标认识当前库且无待执行 → 不覆盖;有待执行 → 回滚将向前补迁移;当前库领先(incompatible)
  ...
  `--restore-db` 直接拒绝。
- 回滚失败同样只告警、事务保留;再次 `--rollback` 续做同一目标(续做时不带 `--no-rescue-snapshot`,救援阶段尚未完成的改为先做救援快照,已结束的保留当时的跳过记录、不补做;反过来不能事后加上);
  下一次正向 `./deploy.sh …` 也会先在同一把锁内自动续做它(已由人确认过),完成后再继续部署。被回滚掉的版本可以 `--code <sha>` 重新部署;只有晋升过的相邻事务才会被提示 `--to`。
