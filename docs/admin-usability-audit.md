...
- **v3.43.2(2026-09-01)前三波补充检视返修:codex 对 v3.40.4/v3.41.0/v3.42.0 的实现
  补做交叉检视**(当时未走检视环节,只有测试+烟测),抓出 7 个存活问题+终审再拦 3 个残余,
  全部修复:高·详情抽屉迟到响应错挂+动作操作错账户(代次守卫+detailUserRef 消旧闭包);
  中·AI 成本闸误拦缓存命中(授权/成本闸分拆,pre_llm_check 下沉缓存 miss 后;TOCTOU 拍板
  软上限记 backlog)、公告假分页(前端 limit=200 本地 slice→真服务端分页)、删号漏三类
  用户名级 KV(同事务清理+LIKE 通配符转义)、审计面板迟到响应覆盖(loadGenRef);
  低·批量审计人数失真(同服务层规范化+数量未知如实记)、审计摘要规则错配(/api/fetch/batch
  精确规则、{article_id:path} 全路径匹配)。前三波其余重点逐项 OK:M01/M04(会话世代
  无旁路)/M05 事务语义(双 admin 同批停用组合正确)/M06/M07/M08/M09。测试 +8 项。
