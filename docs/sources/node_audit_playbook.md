...
4. For browser-rendered detail, run the project's `PlaywrightRenderer` **inside the production container** (`docker compose exec -T backend python -`) against one URL, then two URLs in the same context; a first-pass/then-stuck pattern is a cookie-bound challenge, not a block.
