...
### 十九-补4:按钮规格波(尺寸三档,2026-07-12,主线)

用户目检:节点页「批量运行」(裸 .action-button 40px)与运行页「新建采集任务」
(min-h-[36px] 覆盖)页头高度不一,切页违和;要求全站按钮尺寸审查 + 规格规范
(不强制全一致,但要有档位并遵守)。

审计:全站散布七个高度档(48/40/36/34/32/30/28),min-h 覆盖随手写。定档三级
(conventions §8 硬约束):L=裸 .action-button 40(页头/模态脚/页级批量条/空态 CTA,
页头禁止 min-h 覆盖)、M=min-h-[32px] px-3 text-xs 三件套(卡头/卡内/抽屉脚/设置面板/
工具条)、S=≤28 行内微钮走各自范式类;.icon-button 与 chips 不入档。

并档改动:运行页页头主钮 36→裸 40(点名问题);台账详情抽屉脚 36×3→32;文章详情模态
34→32;技能包下载 34→32;设置账户区 30×3→32;向量雷达检索行 48×3→40(48 档退役);
运维「新建读者」卡头裸 40→32、两模态脚去 text-xs(L 档 14px 字)。设置数据同步区裸→32。

验收:三页页头按钮 computed 全 40;下载技能包/立即生成日报/新建读者全 32;零 pageerror。

### 二十、运维页目检波(四项,2026-07-12,主线)

- **活跃用户 Top 右侧空白**:卡上遗留 maxWidth:620 内联限宽——去除,满幅与 KPI/账户表同宽。
- **「登录 241 次但最近登录 –」数据矛盾**:last_login_at 快照与 LoginEventRecord 事件流
  双轨漂移(快照被历史操作抹掉,事件流完好)。修复取向=读侧兜底而非修库:accounts 服务新增
  `last_login_by_user()`(事件流 group-by max(at)),`/api/admin/accounts` 列表与
  activity 详情的 last_login_at、logged_in_window 一律「快照 or 事件流」;
  回归测试 test_admin_accounts_last_login_falls_back_to_event_stream。
  **原则:快照列只是缓存,事件表才是可靠源,读侧永远要能容忍快照缺失。**
- **新建读者/重置密码弹窗老风格**:旧「well 标题条 + indigo 图标 + soft 脚条」三段横条语法
  退役,沉淀通用**小表单模态语法 .form-sheet**(头/身/脚 hairline 三件套,脚部取消+主钮
  右对齐),两弹窗迁入;规范入 conventions §8。
- **搜索框与按钮不等高**:输入框规格审计定**两档**——表单档 .form-input 36、
  行内工具档 .form-input-inline 32(与 M 档按钮/segmented 同排等高);运维搜索框迁入,
  board-search 34→32 同轴;catalog-search 确认死类登记。

验收:Top 卡宽=页宽(1455);user 最近登录显示 07-11 21:25;搜索框/新建读者同 32;
弹窗 form-sheet 语法截图目检干净;pytest admin_ops+accounts 25 过;零 pageerror。
