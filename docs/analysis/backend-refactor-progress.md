...
**最近更新**：2026-07-02 · 当前分支 `refactor/phase0-concurrency` · 测试基线 **281 passed**
...
| D8 `is_vectorized` 升级状态枚举 pending/indexing/indexed/failed/stale | ⏸️→阶段3 | — | 其 indexing/failed 态与持久化 jobs 强耦合，顺延到阶段3 绑定 jobs 结果一起做 |
...
| 迁移：批量 fetch active sources、collection job run | ⬜ **← 下一增量** | — | 即时触发端点仍 `await run_collection_items(...)` 占长请求 |
| D9 进度持久化（pipeline/progress.py + 日报 _PROGRESS → DB） | ⬜ | — | 目前仍内存态、重启即丢、多实例不可见 |
...
| D10 auth.secret 非默认/不从口令派生（移除回退） | ⬜ | 现仍从口令派生（`app.py` AUTH_SECRET 回退，已抽到 `api/tokens.py`） |
| D10 CORS 域名白名单 / 生产默认 CA 校验 / 密钥迁 env 或 secret manager | ⬜ | |
