...
- 前端 `components/admin/AccountGrowth.jsx`:**一张组合图**——柱 = 当日新增、折线 = 累计账户,各挂一根 y 轴
  (左新增 / 右累计,图例常备;用户验收拍板「两量合一张图」,替代首版的两卡并列),**时间窗直接复用运维页头的
  近 7/14/30/90 天**(首版独立的日/周/月粒度 seg 撤除,用户拍板);逐日桶在前端折算(含空桶),累计从全量序列
  前缀和推出,视野之前的账户算进起点,末端恒等于现存总数。区头 hint「近 N 天 · 新增 X · 现存账户按加入日回溯,已删除不计」。
