...
- 2026-05-13: Added the first collection-job abstraction layer: `NodeGroupRecord`, `CollectionJobRecord`, `CollectionJobRunRecord`, additive `fetch_runs` lineage fields, node-group/job CRUD APIs, task-level run aggregation, legacy single-node scheduled-task compatibility, and frontend controls to save selected nodes as groups or collection jobs. Dify delivery now supports `group_id` and `job_id` source scopes; exact `job_run_id` article delivery remains future work because archived articles do not yet carry per-run lineage.
- 2026-05-13: Added first-ingest article lineage (`fetch_run_id`, `job_id`, `job_run_id`, `source_group_id`, `run_scope`) and threaded it through pipeline saves. Dify delivery now supports `job_run_id`, `fetch_run_id`, and `run_scope` filters for newly saved records; duplicate records skipped by later runs intentionally retain their original lineage.
- 2026-05-25: Completed Stage 2 terminology cleanup for the collector/reader split. The collector UI and API error text now call reusable fetcher sets "采集范围" instead of "节点组"; `NodeGroupRecord`, `group_id`, `source_group_id`, and `/api/node-groups` remain compatibility internals to avoid a destructive schema/API migration.
- 2026-05-25: Completed Stage 3 first archive sync contract. Collector runtimes can export filtered article JSONL through `/api/archive/export/articles.jsonl`; reader runtimes can idempotently import the same format through `/api/archive/import/articles.jsonl`. The contract preserves article identity, content metadata, and first-ingest lineage while treating vector indexes as reader-side derived state.
- 2026-05-25: Completed Stage 4 first reader subscription layer. Reader runtimes now support admin-managed subscription profiles under `/api/subscriptions` and tokenized Dify-compatible consumer delivery under `/api/public/subscriptions/{subscription_id}/dify/articles`; tokens are independent from admin sessions and stored only as HMAC hashes.
- 2026-05-25: Completed Stage 5 UI and deployment polish for the collector/reader split. The reader console now has a `订阅分发` tab for managing subscription profiles, copying Dify pull URLs, and rotating one-time-display consumer tokens; the app header labels the active runtime role, and configuration docs include the external collector plus intranet reader deployment path.
