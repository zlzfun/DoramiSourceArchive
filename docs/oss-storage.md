...
图片原站下载成功、OSS 上传暂时失败时，保留待上传的本地字节和元数据，不记为源站失败。
待上传图片不进入可导出的 `cached` 状态；经过独立的 30 秒冷却后，下一次读取或预取会校验并重试上传，
进程重启后也可复用暂存字节。真正的原站失败继续使用原有负缓存退避。

回收过程中检测到损坏的本地文件时，在独占读取租约下将其隔离为同目录的 `<hash><ext>.corrupt`，
后续读取从 OSS 校验恢复。隔离副本计入实际磁盘用量，不计为新增播客媒体或额外持久配额；
验证远端副本安全后再清理。单个本地文件问题不会阻止其它候选回收，远端故障则停止该轮云端检查。
...
OSS 迁移仅在对象索引为空时允许撤销自身；索引非空时拒绝降级。当前汇合版本还包含不可逆的用户订阅源退役迁移，
回退旧程序须恢复匹配的升级前数据库备份；升级后新增业务数据须另行处理。恢复为 local 不需要降级数据库。
...
- PR #109 初次实现通过后端与前端 CI；完善后的最终结果以 PR 当前提交的检查项为准。
  用户已委托代理完成本地端到端验收；正式检视及修复记录见下节。
  本记录不替代合入或发布门禁。
...
- 与最新主分支集成后，保留 OSS 与用户订阅源退役两条迁移历史，以无 DDL 的汇合节点统一；
  验证从任一分支和已有双版本记录的数据库启动升级，保留原数据并与模型保持一致。
...
### 正式检视与用户委托验收（2026-09-16～17）

- 用户委托的本地验收已通过：桌面亮暗主题、手机宽度图片与音频播放/进度跳转、
  对象服务故障与恢复、实际备份解包和默认 local 接收节点离线读取。
- 检视前冻结提交 `0188a0b416fd178ddb561c674134bbca05d5238c`，共同基线
  `261734016eae3c7d1ca76f01653209dbd39f74d1`；本地与 Linux CI 均为 2043 项后端测试通过，
  前端 28 项测试、lint 与 build 通过。
- 按用户指定，正式审查器为 **Claude Code / `claude-fable-5-1` / `xhigh`**，通过实际 CLI 执行，
  完整审查 54 个变更文件。首轮与协商阶段保持代码冻结，未访问生产数据或调用付费接口。
- 首轮提出 2 项 P2、4 项 P3；六项均经独立隔离复现后接受。先与同一审查会话达成一致，再集中修复，
  后续只对照此清单定点复检。

| 意见 | 复现与协商结论 | 修复方向 |
|---|---|---|
| P2-1 图片上传故障进入源站负缓存 | OSS 恢复后仍被 21600 秒冷却拦截；强制重试会重复回源 | 持久保存待上传元数据，独立短冷却，校验本地字节后重试 |
| P2-2 活跃数据库备份反复重启 | 379.85 MiB WAL 合成库、5 ms 写入间隔：原实现重启 8602 次并于 60 秒超时 | 固定源只读事务，保留分步复制及超时检查 |
| P3-1 本地损坏阻塞整轮回收 | 两轮回收均停在旧损坏文件，归档接口仍提供坏字节 | 独占租约隔离，本地错误继续，云端故障终止，验证后清理隔离副本 |
| P3-2 同步安装丢失规范化 | `.PNG` 会误报清单变化；仅 MIME 参数不会报错，但规范值丢失 | 比较原声明身份，上传后二次核对，再写规范值 |
| P3-3 旧内存状态遮蔽新备份 | 其它实例成功后仍显示旧失败，退避结束多建一份归档 | 新成功快照取代旧内存观察，保留真正失败的退避 |
| P3-4 缺失归档文件返回 500 | 无对象登记且无本地文件时 OSS 路径返回 500；local 对照为 404 | 延迟响应持租约检查缺失；归档 404、代理回源，云故障保留 503 |

备份修复方案实验：固定读事务后，复制约 0.924 秒且无重启，完整备份约 2.706 秒；
单独并发验证中，0.903 秒复制期间另一连接完成 145 次提交。
没有采用单步整库复制，因为它直到整步返回才检查超时；10 ms 预算实验中，分步固定快照约 11.5 ms 中止。
这些是合成环境的正确性实验，不是生产耗时保证。

审查者撤回了“精品导读自动重试导致再次付费”的推断：现有候选筛选排除 `failed`，该路径没有自动重新合成。
付费合成后上传失败时，人工重试可能生成不同口播并产生新费用，保留为既有流程的观察项。
上线前仍需盘点本地缺失文件、检查备份依赖与容量，并实际授权和演练生产备份；本次检视不代表已完成生产切换。

六项均已按协商方案修复。定点验证：备份 25 项、缓存/响应/分享等 68 项、
新增图片重试与同步规范化 36 项、既有媒体/对象存储/同步 103 项通过（分组之间存在交集，不合计为全量数量）。
修后实际 379.85 MiB WAL 备份为 0.925 秒复制、2.642 秒完成，零重启，恢复数据库完整。
全量回归与同一 Claude 会话的最终定点复检结论以 [PR #109](https://github.com/zlzfun/DoramiSourceArchive/pull/109)
当前提交对应的检查和检视记录为准；未完成这些门禁前不合并或发布。
