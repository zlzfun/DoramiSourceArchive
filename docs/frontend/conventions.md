...
**阅读器页头两族(2026-09-06,issue #23)**:阅读器整幅页面的页头只有两种,按页面**主语**选:

| 族 | 用于 | 规格 | 在册类 |
|---|---|---|---|
| 栏名工具头 | 内容容器与站内目录:文章/播客/动态列头、社交头、发现页头 | 标题 14/600,同落 19px 基线;工具行在下 | `.reader-list-title` / `.reader-social-title` / `.reader-disc-title`(同一条规则) |
| 报头 | 「我的」页:我的早报、我的兴趣(以读者本人为主语,占源栏槽位 + 整幅右栏) | 等宽 kicker 11px/0.12em accent-ink + 衬线标题(早报 36 / 兴趣 32)+ 13px muted 提示,版心 1120 | `.brief-mast` 族 |

规则:看到衬线报头即知这是属于读者自己的一面;容器与目录页不得借用报头,「我的」页不得退成工具头。
视图轨分组与之对应:早报·兴趣 | 文章·播客·动态·社交 | 发现——同族相邻。
横向对齐各随下方内容(列头 16 / 社交 24 / 发现随 920 版心),不强求页间对齐。

**新闻价值分的身份线(2026-09-06,issue #23;v3.48 起分数语义=新闻价值)**:分数是数字,不是标签——全站只用一种画法、三档大小,
字体栈恒 `Charter, Cambria, Baskerville, var(--font-serif)`(齐线数字):速读卡 38px 渐变 → 早报卡 22px 渐变 →
阅读器条目行 13px 渐变(`.reader-entry-score.ai-grad-text`,落元信息行时间之前;三档同字同色,「见渐变即 AI」在列表同样成立)。
不做分数胶囊。**分数按分值分档着色(2026-09-10,issue #54,样页 `dorami-score-tiers-quiet.html`)**:只改颜色,三档大小与字体栈不动;
灰线 6.0 以下退成与旁边元信息同一个 `--dorami-faint`、不画身份色,其上按整数分一档(`is-score-1..4`,渐变两端色各自向 faint
`color-mix` 85/60/30/0%),9+ 完整渐变——「有渐变 = 值得看」本身是信息,「见渐变即 AI」不破。灰线取自**评分尺子自身的档界**
(提示词把 1.0–5.9 定为泛泛/宣传/重复、6.0 起才「有用或深入」),是固定视觉刻度,随尺子版本走、**不随运营旋钮
`daily_brief_min_score` 联动**(与台账筛选 7+/8+/9+ 固定档位同口径;评分尺子换版本改分布时随之重定灰线)。档位判定单点
`utils/analysis.scoreTierClass`,管理面纯色触点复用同一比例把 accent-ink 向 faint 混、不上渐变。条目行的分类(`.reader-entry-tag`)是源名后的一段
10px 细线小签(沿用「动态」形态签 `.reader-shape-chip` 的语法:hairline 框、字距 0.06em、faint 不着色),
靠形态与来源分开;阅读窗标题区的标签行同为细线小签(规范实线 / 灵活虚线可点),饱和胶囊只留早报卡。
**阅读窗标题区四行**(2026-09-06,样页 `dorami-pane-head-quiet.html`):眉头「源 · 体裁」(等宽 accent,报头家族;
内容类型是数据形态不上眉头)/ 衬线标题 / 署名行只放事实(日期——无真实时刻不画 00:00 · 阅读约 N 分钟 · 阅读量 ·
分析状态 faint 文字,中点分隔)/ 尾行「左标签小签 · 右动作(语言二段 + 查看原文文字链)」贴着分隔线。
三种语言(等宽 + 衬线 + 小签),饱和色只在眉头,渐变留给速读卡与译文激活态;往标题区加东西先问它归哪一行。条目行只有三档音量:
标题响、摘要轻、元信息最轻——新增信息先问它属于哪一档,不给卡片加行。
...
一个分组列表里的所有标签必须回答**同一个问题**。混轴是常见病:源栏曾把
「官方/媒体/个人」(信息角色:谁在说)和「榜单·动态/社交·账号」(内容形态:什么形式)
并列在一个列表,两个正交维度混在一起,逻辑不齐。

**当某个维度已被更高层结构承担,就不该在下层再作分组维度。** 形态(文章/动态/社交)
一旦上升为容器(视图轨)或过滤条(发现页 seg),源栏组头再标形态就是重复
——「动态」容器里的「榜单·**动态**」、「社交」容器里的「社交·**账号**」都是把容器名又说一遍。
剥掉形态后,分类统一到单一的**信息角色**轴(官方/媒体/个人/榜单),阅读器源栏、
发现页、管理面共用一套词汇(`sourceRoleOf`/`SOURCE_ROLES`,单一事实来源在 `sourceTaxonomy.js`)。

标签文字忌虚词与自夸:「官方·一手信息」的「一手」是官方的应有之义、「个人·洞见」
预设了每条个人内容都有洞见——精简到「官方」「个人」,信息量反而更高。

## 4.1.2 导航是目的地,过滤是修饰符——别混在一条轨上

与「单轴」同源的另一种混轴:把**修饰符**塞进**导航**列表。阅读器视图轨的
文章/动态/社交/发现都是**目的地**(一级视图,互斥,占满 条目列+阅读窗,回答「我在哪」);
搜索不是目的地,而是叠加在当前容器上的**过滤器**(回答「筛掉什么」)——它曾被摆成
视图轨的第五个钮,既是混轴,又派生出「发现页点搜索反而跳回上个容器」这类怪相
(搜索依附容器,发现页本无它的位置)。

**判据:这个控件是"能去到的地方",还是"作用在当前内容上的开关"?** 前者进导航轨,
后者归内容区的过滤器带。搜索因此降级到条目列头,与 全部/未读、标读、收藏 并排——
那一带本就是"过滤当前容器"的控件集合。防拥挤靠**就地展开**:静止态只是一个紧凑图标,
点开时输入框顶替标题+未读 seg,控件数不增反匀(见 `ReaderTab.jsx` 列头 + `.reader-search-inline`)。
...
- **微型分段 `.mini-seg` / `.mini-seg-btn`(`.is-on`=白拇指)**:表脚/工具条级轻量二择(如表格密度 舒适/紧凑),勿再造小开关。**播放器内「未就绪态动作」优先占用既有分段槽位**(如播客点播精品导读占「精品导读」位:可点播 → 生成中… → 已就绪切换),不要另开整宽次按钮。
...
- **工作区（静默仪器，2026-07）：动效 = feedback-only。** 仪式性入场已整体拆除
  ...
  **具名例外·AI 速读卡分数入场（v3.45.1，用户拍板保留，codex 检视 P1 提出后仍拍板）**：阅读窗
  速读卡的新闻价值分以「里程表」方式入场——每位数字住定宽格，格内 0–9 竖条 transform 2.2s 滑到
  分值、十分位晚 220ms 起步（`.reader-ai-odo-*`，`AiReadingCard.jsx`）。它是入场编排而非反馈，且
  滚动期间数字是过渡值——这两点与本节原则相抵，登记为产品决定：这张卡是全站唯一的 AI 身份触点，
