...
- ☐ **统一新闻价值评分波**(2026-09-05 讨论定向,方案 `docs/unified-news-scoring-plan.md`):
  文章级评分单尺子化(重要性,不再量阅读价值)+ 公共日报复用入库分析(map 逐篇打分退役,
  只为入选篇写点评/中文标题;候选无分析结果则就地无上限补评)。动工前先读生产 shadow 指标、
  手标黄金集,并过 §3 待拍板清单。
