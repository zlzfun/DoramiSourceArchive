...
- ☐ **issue #33 个人早报产品优化波**(v3.50.x):§2 重大事件通道已落地(v3.50.0,方案
  ...
  §5 兴趣/订阅变更不触发重编已落地(v3.50.1,`docs/personal-brief-rebuild-entrypoints.md`);
...
- ◇ **兴趣走出早报**(issue #27,2026-09-06 分析 #23 兴趣页改页面时立项):读者兴趣(关注/屏蔽
  ...
  实体不合并,统一页面语言(#23)与二者之间的桥(#27)。扩展方向按代价:列表侧命中高亮/降灰 →
  「只看关注」档位 → 兴趣驱动源发现(源×标签热度矩阵,需聚合端点)→ 问答先验 → feed/MCP 屏蔽
  (改交付契约,单独决策);口径:未打标一律显示、缺分析是缺席不是不命中。逐项拍板后动工。
