...
- ☐ **issue #33 个人早报产品优化波**(v3.50.x):§2 重大事件通道已落地(v3.50.0,方案
  ...
  余下 §5 兴趣变更不触发重编(纯收口,先出)→ §4 中文标题(翻译缓存复用)→ §3 页面「个性化/智能化」(样页先行)。
...
- ◇ **兴趣走出早报 · 后续波次**(issue #27 第一波 v3.51.0 已落地,方案 `docs/interest-lens-wave-plan.md`):
  已做=左栏三谓词过滤面板(订阅 / 兴趣 / 收藏,AND 联合,全关 = 全站)+ 命中胶囊 / 屏蔽折叠行 /
  订阅外就地订阅 + 兴趣编辑面并入发现页第三段。**未做,逐项等数据或拍板**:列头下逐标签 chip 收窄
  (2026-09-09 拍板暂缓);发现页标签卡「主要来源」脚部(不带数字的桥,可选);兴趣渠道准入门槛是否再加
  新闻价值分下限(现只认主标签或相关度 ≥ 0.8);早报兴趣半是否取全站;feed / MCP 屏蔽(改交付契约);
