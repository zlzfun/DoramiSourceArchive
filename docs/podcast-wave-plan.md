...
- 应用启动时会幂等安装 37 个共享 Podcast `SourceConfigRecord`（不替用户订阅），使全新部署的“节点管理”直接具备播客目录；节点展示健康、累计单集、运行参数和与博客相同的源审查，并支持立即抓取、批量运行、存为采集任务和读者面隐藏。Podcast 与博客一样作为逻辑节点进入统一 `CollectionJobRecord`，任务成员和任务状态决定是否运行，并共用任务 Cron、运行历史和参数覆盖；执行时解析为隐藏的 `generic_podcast_rss` 模板。采集只更新 RSS 元数据，不直接触发 ASR/TTS。Issue #153 的 Tessl 节目另有启用的观察期任务和兼容 entry namespace，用于无损收养同 feed 的存量自定 Podcast。
...
- **Issue #44 已完成**：发布方逐字稿优先、无稿时 ASR 回退，完整逐字稿经分块归约产出全文
  终评；处理任务可恢复、可重试，全文结果替换阅读面当前分。
- **Issue #45 已完成，issue #97 调整现行策略**：简介付费 ASR 处理线固定 `>= 6.0`；有发布方
  逐字稿时不看简介分直接全文分析；全文优质门槛以运行时 KV 持久化，默认 `>= 7.5`，支持
  `1.0–10.0` 一位小数。分析表分别保留简介分与全文分，资格、统计、徽标
  只读取全文分。运维内容页提供四项轻量统计、六类状态筛选、原因解释、强制全文与失败
  重试；调门槛只即时重算历史全文资格/待生成，历史博客与音频原样保留。
- 已将现有 Podcast 分析统一标成“简介初评”，并只使用描述性的 `is_long_form`。
- 简介初评与全文终评共享播客评分尺但使用独立输入提示词；人物知名度与公共影响力是正向
  信号，议题相关权威和一手信息会放大加分，只有名气而缺乏实质内容不能单独达到 8 分。
- 两段评分各自承担一种决策：简介初评只作为付费 ASR 回退的成本门（`>= 6.0`，
  `5.9` 不通过、`6.0` 通过，管理员可强制绕过）；发布方逐字稿是零 ASR 的独立全文候选，
  不受简介分限制。发布方逐字稿或 ASR 全文完成后重新评分，该全文终评才按优质门槛（默认
  `>= 7.5`）决定“优质播客”资格及精品导读生成。两分不加权、
