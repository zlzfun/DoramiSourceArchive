...
数据库复制在专用只读连接的固定读事务中分步进行，包含该快照建立时已经提交的记录；
后续提交不进入这份快照，也不会使复制从头重来。WAL 模式下写入可以继续，但检查点受快照约束，
备份期间 WAL 可能增长；非 WAL 模式下，读事务可能暂时阻塞写入提交。
`timeout_seconds` 同时用于 OSS 连接超时和数据库复制阶段的超时检查，默认 60 秒；
它不是整份归档打包、校验及上传的总时限。复制仍分批检查截止时间，超时后释放读事务和回执锁。
机制参考 [SQLite 在线备份的连接与锁](https://www.sqlite.org/backup.html#file_and_database_connection_locking)。
...
另一进程写入更新的成功快照后，旧的内存忙碌或失败状态会让位于该成功记录，避免误报失败或重复备份。
