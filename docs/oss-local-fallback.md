...
OSS 分支的 Alembic 迁移 `c92a8f01d6b3` 仅在 `object_blobs` 为空时允许撤销自身；有任何对象索引时拒绝降级。
当前发布迁移通过 `d17e9a4c2b61` 同时包含用户订阅源退役迁移 `c8a1e4f7d2b6`，后者不支持降级。
因此，从当前汇合版本回退旧程序须恢复匹配的升级前数据库备份，不能把「OSS 索引已清空」视为整库可降级。
上述恢复为 local 的流程不需要数据库降级。不要手工清空对象索引来绕过文件恢复与校验。
