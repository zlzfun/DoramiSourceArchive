...
// 统一的确认弹窗 Provider：替代原生 window.confirm，复用共用 Modal 外壳(issue #104 迁入:
// 遮罩关闭判定 / Esc / 焦点陷阱与归还 / body 滚动锁 / 退场动画全由外壳承担)。
...
export function ConfirmProvider({ children }) {
  const [state, setState] = useState(null);      // 文案载荷:退场动画期间仍需渲染,故关闭后不清
  const [visible, setVisible] = useState(false); // 外壳的 open;false 后由外壳播完退场再卸载
  const resolverRef = useRef(null);

  // 立即 resolve（不阻塞调用方）；退场交给外壳。
  ...
  // 仅 Esc 取消(外壳 useModalA11y 承担)，不绑定 Enter 确认，避免误触危险操作。
  return (
    <ConfirmContext.Provider value={confirm}>
      {children}
      {state && (
        <Modal
          open={visible} onClose={() => settle(false)} closeOnOverlay centered size="md"
          role="alertdialog" ariaLabel={state.title} overlayClassName="z-[100]"
        >
