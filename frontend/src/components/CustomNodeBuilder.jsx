  ...
  return (
    <div className="space-y-6">
              ...
              <ul className="space-y-2">
                {(preview.entries || []).map((e, i) => (
                  <li key={i} className="text-sm">
                    ...
                    {e.method && <span className="ml-2 text-[10px] font-mono text-slate-400">{e.method}</span>}
      ...
      <div className="surface-card rounded-[var(--r-overlay)] p-5">
        ...
        {saved.length === 0 ? (
          <div className="text-sm text-slate-400">暂无自定义源。用上方功能分析一个 URL 并保存即可。</div>
        ) : (
            ...
            {saved.map(row => (
              <li key={row.source_id} className="flex items-center gap-3 py-2.5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="truncate text-xs text-slate-400 font-mono">{row.source_id} · {row.source_type} · {row.url}</div>
                </div>
