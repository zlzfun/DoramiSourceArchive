      ...
      <div className="bg-white rounded-3xl w-full max-w-4xl shadow-2xl flex flex-col max-h-full overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          <div className="flex items-center space-x-3">
            ...
            <span className="px-2 py-0.5 rounded bg-blue-100 text-blue-700 text-xs font-bold">{data.content_type}</span>
          </div>
          <div className="flex items-center space-x-2">
            <button onClick={onToggleEdit} className={`px-3 py-1.5 rounded-lg text-sm font-bold flex items-center transition-colors ${isEditing ? 'bg-amber-100 text-amber-700' : 'bg-slate-200 text-slate-700 hover:bg-slate-300'}`}>
              {isEditing ? <X className="w-4 h-4 mr-1" /> : <Edit2 className="w-4 h-4 mr-1" />}
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-5 bg-white">
          <div>
            ...
            {isEditing ? (
              <input type="text" defaultValue={data.title} id="edit-title" className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg font-bold outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" />
            ) : <div className="font-extrabold text-xl text-slate-800 leading-snug">{data.title}</div>}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              {isEditing ? (
                <input type="text" defaultValue={data.source_url} id="edit-url" className="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:border-blue-500" />
              ) : (
                data.source_url ?
          ...
          <div>
            ...
            {isEditing ? (
              <textarea defaultValue={data.content} id="edit-content" rows="8" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:border-blue-500 leading-relaxed" />
            ) : <div className="text-sm bg-slate-50 p-4 rounded-2xl border border-slate-100 whitespace-pre-wrap leading-relaxed text-slate-700 shadow-inner max-h-64 overflow-y-auto">{data.content || '无正文内容'}</div>}
          </div>
          ...
          <div>
            ...
            {isEditing ? (
              <textarea defaultValue={data.extensions_json} id="edit-extensions" rows="6" className="w-full p-3 bg-slate-800 text-emerald-400 border border-slate-700 rounded-lg font-mono text-xs outline-none focus:ring-1 focus:ring-blue-500" />
            ) : <pre className="text-xs bg-slate-800 text-emerald-400 p-4 rounded-2xl overflow-x-auto shadow-inner">{JSON.stringify(JSON.parse(data.extensions_json || '{}'), null, 2)}</pre>}
          </div>
        </div>
        ...
        {isEditing && (
          <div className="p-4 bg-slate-50 border-t border-slate-200 flex justify-end space-x-3">
            ...
            <button onClick={onToggleEdit} className="px-5 py-2 rounded-lg font-bold text-slate-600 bg-white border border-slate-300 hover:bg-slate-50 transition-colors">取消</button>
            <button onClick={handleSave} className="px-5 py-2 rounded-lg font-bold text-white bg-blue-600 hover:bg-blue-700 flex items-center transition-colors shadow-md">
