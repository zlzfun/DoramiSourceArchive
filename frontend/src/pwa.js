...
let installEvent;
let snapshot = { installed: media.matches || navigator.standalone === true, canPrompt: false, updateAvailable: false, online: navigator.onLine };
...
// UA is only a hint for instructions, never proof of installation support.
export function installGuide(nav = navigator, secure = window.isSecureContext) {
  ...
  if (/HuaweiBrowser|HarmonyOS/i.test(ua)) return 'huawei';
