...
补丁节点:3.23.1 阅读器滚动条修复——文章/动态中栏经发现页或社交流条件卸载后重建时，自绘滑块重新绑定新 DOM，修复内容滚动但滑块停滞的偶现问题。3.23.2 Meta AI 列表解析修复——按单文章边界跳过任意语言 CTA 并固定请求语言，从详情页回退标题/发布日期；Meta 显式开启权威元数据刷新，再抓可修正已归档的错标题或抓取时间伪日期，正文不覆盖。3.23.3 动态空态口径统一 + 预置订阅补动态/社交各一源。
