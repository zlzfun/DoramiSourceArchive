...
| [`storage_backup.py`](./storage_backup.py) | 创建、查看、校验、下载备份并离线恢复，见 [说明](../docs/storage-backups.md)。 | 默认关闭；`create` 显式启用后写私有快照并可上传独立 OSS 前缀；不调用付费生成接口。恢复需已知 SHA-256、`--offline` 与空目录，不覆盖原生产路径。 |
