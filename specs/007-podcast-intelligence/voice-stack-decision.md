...
**Status**: Proposed for benchmark; product output contract accepted
...
| 场景 | 首轮主候选 | 选择理由 | 备选/基准 |
|---|---|---|---|
| 国内生产链 | 阿里 Qwen-Audio 3.0 文件转写；腾讯/阿里合规中文平台旁白 A/B | 中文、长音频、热词、说话人和公开价格组合完整；国内网络与数据驻留更简单 | Paraformer-v2 低成本 ASR；Azure 旁白作为质量基准 |
...
不能只凭公开单价选唯一供应商。首轮以 10 集真实播客做盲测，满足质量门后才在同档中选便宜者。基于当前公开能力，**国内最值得先做 PoC 的是阿里 ASR + 腾讯/阿里 TTS；全球最稳妥且价格透明的是 AssemblyAI ASR + Azure TTS**。国内两家公开页的输出音频再分发条款不如 Azure 清晰，上线必须以企业合同确认结果为硬门。
...
```text
60min Qwen-Audio 3.0 ASR       ¥0.792
3200 汉字 CosyVoice v3 Flash   ¥0.640  # 按 6400 计费字符，使用系统音色
文本翻译/摘要/校验              以实际 LLM token 计
-------------------------------------------------
语音部分小计                   ¥1.432 / 集
```

若合同确认腾讯通用大模型音色可按目标范围分发，则 TTS 样本约 ¥0.384，Qwen ASR + 腾讯 TTS 的语音部分约 ¥1.176/集。低成本纯中文 ASR 用 Paraformer-v2、TTS 用腾讯时可到约 ¥0.672/集，但只有真实样本达到专名、说话人和中英混读门槛时才能启用。

### 全球链公开价样本

```text
60min AssemblyAI U3.5 Pro      $0.210
speaker labels                 $0.020
3200 汉字 Azure Neural TTS     $0.096  # 按 6400 计费字符
文本翻译/摘要/校验             以实际 LLM token 计
-------------------------------------------------
语音部分小计                   $0.326 / 集
```

现有 `$0.35` 单集软预算是可努力达到的目标，但在完整中文翻译、多轮摘要或失败重试发生时会偏紧；`$0.60` 继续作为无需人工升级模型时的硬上限。成本控制优先级依次是：官方稿免 ASR、只处理相关且高价值节目、TTS 按需生成、失败只重做单段、最后才考虑自托管。
